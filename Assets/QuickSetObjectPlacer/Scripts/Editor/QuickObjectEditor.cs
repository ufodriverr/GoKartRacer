...
//				}
	...
	void PlaceGameObject ()
	{
		   ...
		   PrefabUtility.GetPrefabType(newObj) == PrefabType.ModelPrefab)
		{
			...
			go.transform.rotation = Quaternion.Euler(new Vector3(UnityEngine.Random.Range(0f,rotation.vector3Value.x),UnityEngine.Random.Range(0f,rotation.vector3Value.y),UnityEngine.Random.Range(0f,rotation.vector3Value.z)));
			go.transform.localScale = Vector3.one*UnityEngine.Random.Range(scale.vector3Value.x,scale.vector3Value.y);
		...
		{
			Object objRoot = PrefabUtility.GetCorrespondingObjectFromSource(newObj);
